...
export default function GlobalTrashPanel({ open, onClose, onRestored }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    db.entities.TrashedItem.filter({ item_type: 'dashboard' }, '-created_date', 50)
      .then(setItems)
  ...
  const handleRestore = useCallback(async (item) => {
    ...
    try {
      ...
      // Create dashboard + delete trash entry concurrently
      await Promise.all([
        db.entities.Dashboard.create(rest),
        db.entities.TrashedItem.delete(item.id),
      ]);

      // Notify parent to update dashboard list (lightweight invalidation)
      onRestored?.();
    } catch (e) {
      // Rollback
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold">Deleted Dashboards</h2>
                {items.length > 0 && (
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-48 text-center">
                  ...
                  <p className="text-sm font-medium text-muted-foreground">No deleted dashboards</p>
                  <p className="text-xs text-muted-foreground/60 mt-1">Deleted dashboards will appear here</p>
                </div>
              ) : (
                  ...
                  {items.map(item => (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium truncate">{item.item_name || 'Untitled Dashboard'}</p>
